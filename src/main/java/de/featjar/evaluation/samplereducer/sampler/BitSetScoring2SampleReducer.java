...
public class BitSetScoring2SampleReducer implements ISampleReducer {
    ...
    private static class Config extends BooleanSolution {
        ...
        public synchronized void incScore(int s) {
            score += s;
            interactionCount++;
        }

        public synchronized void decScore(int s) {
            score -= s;
            interactionCount--;
        }
        ...
        while (!interactions.isEmpty()) {
            ...
            for (int j = masterBitSet.nextSetBit(0); j >= 0; j = masterBitSet.nextSetBit(j + 1)) {
                ...
                if (config.score <= 0 || config.interactionCount == 0) {
                    masterBitSet.clear(j);
                } else if (config.interactionCount / (double) config.score > bestScore) {
                    bestScore = config.interactionCount / (double) config.score;
                    bestConfigIndex = j;
                }
            }
