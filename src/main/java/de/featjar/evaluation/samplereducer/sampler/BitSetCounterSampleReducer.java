...
public class BitSetCounterSampleReducer implements ISampleReducer {

    private static class Config extends BooleanSolution {
        private static final long serialVersionUID = 8975357089309744380L;
        ...
        while (!interactions.isEmpty()) {
            ...
            for (int j = masterBitSet.nextSetBit(0); j >= 0; j = masterBitSet.nextSetBit(j + 1)) {
                ...
                if (config.interactionCount == 0) {
                    masterBitSet.clear(j);
                } else if (config.interactionCount > bestScore) {
                    bestScore = config.interactionCount;
                    bestConfigIndex = j;
                }
            }
